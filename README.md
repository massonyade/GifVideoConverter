Copyright (c) 2026 まそん
All rights reserved.

【日本語訳】
本リポジトリのソースコードは、安全性の確認および監査（オーディット）目的での「閲覧」のみを許可しています。
著作者の書面による事前の書面合意がない限り、本ソフトウェアおよび関連ソースコードの「複製」「改変」「再配布」「商業的または個人プロジェクトへの利用（実行）」を行うことは一切禁止します。
